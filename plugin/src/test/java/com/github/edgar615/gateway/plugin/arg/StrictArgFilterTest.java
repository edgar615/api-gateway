...
public class StrictArgFilterTest {
    ...
    public void unDefinedUrlArgPluginShouldFailed(TestContext testContext) {
        ...
        Filters.doFilter(task, filters)
            ...
            testContext
                    .assertEquals("prohibited", e.getErrorDetail().get("appKey").iterator().next());
            testContext
                    .assertEquals("prohibited", e.getErrorDetail().get("sign").iterator().next());
            async.complete();
        });
    }
    ...
    public void unDefinedUrlArgPluginButExcludeArgShouldSuccess(TestContext
        ...
        filter =
                Filter.create(StrictArgFilter.class.getSimpleName(), Vertx.vertx(), new JsonObject()
    ...
    public void unDefinedBodyArgPluginShouldFailed(TestContext testContext) {
        ...
        Filters.doFilter(task, filters)
            ...
            ValidationException e = (ValidationException) t;
            testContext
                    .assertEquals("prohibited", e.getErrorDetail().get("type").iterator().next());
            testContext
                    .assertEquals("prohibited", e.getErrorDetail().get("name").iterator().next());
            async.complete();
        });
    }
