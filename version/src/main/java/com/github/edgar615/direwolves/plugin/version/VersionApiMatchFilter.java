 ...
 */
public class VersionApiMatchFilter implements Filter {
  ...
  public VersionApiMatchFilter(Vertx vertx, JsonObject config) {
    this.vertx = vertx;
  ...
  private ApiDefinition matchApi(List<ApiDefinition> apiDefinitions, String reqVersion) {
    ...
    List<ApiDefinition> grayDefinitions = apiDefinitions.stream()
            .filter(d -> d.plugin(VersionMatchPlugin.class.getSimpleName()) != null)
    ...
    VersionMatchPlugin versionMatchPlugin =
            (VersionMatchPlugin) grayDefinition.plugin(VersionMatchPlugin.class.getSimpleName());

    if (Strings.isNullOrEmpty(reqVersion)) {
      //没有请求头，使用默认配置
      return matchDefault(apiDefinitions, versionMatchPlugin);
    } else {
      return matchWithVersion(apiDefinitions, reqVersion, versionMatchPlugin);
    }
  }

  private ApiDefinition matchWithVersion(List<ApiDefinition> apiDefinitions, String reqVersion,
                                         VersionMatchPlugin grayPlugin) {
