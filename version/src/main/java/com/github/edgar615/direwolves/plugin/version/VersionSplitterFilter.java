...
public class VersionSplitterFilter implements Filter {
  ...
  public boolean shouldFilter(ApiContext apiContext) {
    return apiContext.apiDefinition() == null
           && apiContext.variables().containsKey("request_clientIp");
  }
  ...
  public void doFilter(ApiContext apiContext, Future<ApiContext> completeFuture) {
    ...
    apiFinder.getDefinitions(apiContext.method().name(), apiContext.path(), ar -> {
      ...
      try {
        ...
        ApiDefinition apiDefinition = matchApi(apiDefinitions, clientIp);
  ...
  private ApiDefinition matchApi(List<ApiDefinition> apiDefinitions, String clientIp) {
    ...
    return matchWithVersion(apiDefinitions, clientIp, plugin);
  }

  private ApiDefinition matchWithVersion(List<ApiDefinition> apiDefinitions, String clientIp,
                                         VersionSplitterPlugin plugin) {
    Optional<IpSplitPolicy> optional =
            plugin.policies().stream()
    ...
    List<ApiDefinition> matchDefinitions = apiDefinitions.stream()
            .filter(d -> {
              ...
              return versionPlugin != null
                     && versionPlugin.version().equalsIgnoreCase(optional.get().version());
            })
            .collect(Collectors.toList());
