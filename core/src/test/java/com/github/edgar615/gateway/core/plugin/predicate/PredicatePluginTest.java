...
public class PredicatePluginTest {
  ...
  public void testDecode() {
    JsonObject predicate = new JsonObject()
            ...
            .put("between", new JsonArray().add("2018-08-05T17:05:02.717+08:00").add("2018-08-06T17:05:02.717+08:00"));
    ...
    ApiPlugin plugin = new PredicatePluginFactory().decode(new JsonObject().put("predicate", predicate));
  ...
  public void testEncode() {
    ...
    plugin.add(between);
    HeaderContainsPredicate headerContainsPredicate = new HeaderContainsPredicate(Lists.newArrayList("X-Api-Version", "Content-Type"));
    plugin.add(headerContainsPredicate);
    HeaderEqualsPredicate headerEqualsPredicate = new HeaderEqualsPredicate(ImmutableMap.of("foo", "bar"));
    plugin.add(headerEqualsPredicate);
    HeaderRegexPredicate headerRegexPredicate = new HeaderRegexPredicate(ImmutableMap.of("foo", "\\\\d+"));
    plugin.add(headerRegexPredicate);
    QueryContainsPredicate queryContainsPredicate = new QueryContainsPredicate(Lists.newArrayList("X-Api-Version", "Content-Type"));
    plugin.add(queryContainsPredicate);
    QueryEqualsPredicate queryEqualsPredicate = new QueryEqualsPredicate(ImmutableMap.of("foo", "bar"));
    plugin.add(queryEqualsPredicate);
    QueryRegexPredicate queryRegexPredicate = new QueryRegexPredicate(ImmutableMap.of("foo", "\\\\d+"));
    plugin.add(queryRegexPredicate);
    RemoteAddrAppointPredicate addrAppointPredicate = new RemoteAddrAppointPredicate(Lists.newArrayList("192.168.0.100"));
