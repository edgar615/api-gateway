...
import com.github.edgar615.gateway.dispatch.BaseHandler;
import com.github.edgar615.gateway.dispatch.DispatchHandler;
import com.github.edgar615.gateway.dispatch.FailureHandler;
...
public class ApiDispatchVerticle extends AbstractVerticle {
    ...
    private void startHttpServer(Router router, Future<Void> startFuture) {
        ...
        vertx.createHttpServer(options)
                ...
                .listen(config().getInteger("port", port), ar -> {
                    if (ar.succeeded()) {
                        LOGGER.info("ApiDispatchVerticle deploy succeeded, namespace:{}, port",
                                    namespace, port);
                        startFuture.complete();
                    } else {
                        LOGGER.error("ApiDispatchVerticle deploy failed, namespace:{}, port",
                                     namespace, port, ar.cause());
