  ...
  public void testRpc(TestContext testContext) {
    ...
    future.setHandler(ar -> {
      if (ar.succeeded()) {
        ...
        JsonObject result = apiContext1.result().getJsonObject(0);
        System.out.println(apiContext.result());
        Assert.assertEquals(result.getString("id"), config.getString("id"));
      } else {
        testContext.fail();
      }
    });;
