...
public class HttpRequestReplaceFilterTest {
    ...
    public void testReplaceParamsFromHeader(TestContext testContext) {
        ...
        Filter filter =
                Filter.create(HttpRequestReplaceFilter.class.getSimpleName(), vertx,
                              new JsonObject());
        filters.add(filter);
    ...
    public void testReplaceParamsFromBody(TestContext testContext) {
        ...
        Filters.doFilter(task, filters)
                .andThen(context -> {
                    ...
                    testContext
                            .assertEquals("{\"foo\":\"bar\"}", request.params().get("q4").iterator
                                    ().next());
                    async.complete();
                }).onFailure(t -> {
            t.printStackTrace();
    ...
    public void testReplaceHeadersFromBody(TestContext testContext) {
        ...
        Filters.doFilter(task, filters)
                .andThen(context -> {
                    ...
                    testContext
                            .assertEquals("{\"foo\":\"bar\"}", request.headers().get("h4").iterator
                                    ().next());
                    async.complete();
                }).onFailure(t -> {
            t.printStackTrace();
    ...
    public void testReplaceBodyFromHeader(TestContext testContext) {
        ...
        Filters.doFilter(task, filters)
                .andThen(context -> {
                    ...
                    testContext.assertEquals("h1.1",
                                             request.body().getJsonArray("b1").iterator().next());
